...
The `onLoad` callback receives a `defer` function, which returns a `Promise` that resolves once Bun has loaded all _other_ modules. Await it when a module's contents depend on other modules.
...
You can call the `.defer()` function only once per `onLoad` callback, and only while Bun is still loading that module. A `defer` you kept around and call after the callback has produced its result throws.
