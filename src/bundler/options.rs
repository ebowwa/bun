...
/// `bake_types` (and through it `bun_runtime::bake`) re-exports this as the
/// one nominal type; the runtime projects its canonical `bake.Framework`
/// superset into this view via `as_bundler_view`. `file_system_router_types`
/// stays in the runtime because only `bake::FrameworkRouter` reads it.
#[non_exhaustive]
pub struct Framework {
    pub(crate) built_in_modules: StringArrayHashMap<bun_options_types::BuiltInModule>,
