                            ...
                            break 'needs_install match &patch_info {
                                installer::PatchInfo::None => {
                                    if !exists {
                                        ...
                                        // `bun unlink` has no uninstall step: a
                                        // `.bun-link` marker (dropped by the
                                        // link override in Installer.rs) with
                                        // no active link means the body still
                                        // holds the producer's files — rebuild
                                        // from the registry. Only reached when
                                        // `has_active_link` is false (it
                                        // short-circuits the chain above).
                                        // Entries that regain global-store
                                        // eligibility rebuild via the global
                                        // existence miss; this covers ones
                                        // that stay project-local (trusted
                                        // deps, `globalStore` off — the
                                        // default). Mirrors the
                                        // `PatchInfo::Remove` un-patch
                                        // recovery.
                                        store_path.set_length(scope_for_patch_tag_path);
                                        store_path
                                            .append(installer::LINK_OVERRIDE_MARKER)
                                            .assume_ok();
