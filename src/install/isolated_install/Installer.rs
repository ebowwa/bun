...
/// Dropped inside a store entry's package body when it was materialized from
/// a `bun link` producer instead of the registry. `bun unlink` has no
/// uninstall step, so the next install's `needs_install` check (in
/// isolated_install.rs) uses this to detect a producer-sourced body whose
/// link is gone and rebuild it, mirroring how `PatchInfo::Remove` recovers an
/// un-patched entry.
pub(crate) const LINK_OVERRIDE_MARKER: &[u8] = b".bun-link";
                        ...
                        tag => {
                                ...
                                ) != InstallMethod::Symlink
                            {
                                ...
                                if let Some((ptr, len)) = producer_path_opt {
                                    ...
                                    // Failing the task on a marker write error
                                    // (rather than ignoring it) keeps the
                                    // unlink-recovery guarantee: without the
                                    // marker a later unlinked install would
                                    // keep this producer body forever.
                                    let mut marker = AutoPath::init();
                                    installer.append_store_path(&mut marker, self.entry_id);
                                    marker.append(LINK_OVERRIDE_MARKER).assume_ok();
                                    if let sys::Result::Err(err) = sys::File::openat(
                                        Fd::cwd(),
                                        marker.slice(),
                                        sys::O::WRONLY
                                            | sys::O::CREAT
