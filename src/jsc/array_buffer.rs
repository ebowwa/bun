...
pub fn typed_array_from_owned_slice(
    ...
    debug_assert!(
        ty != TypedArrayType::TypeNone && ty != TypedArrayType::TypeDataView,
        "typed_array_from_owned_slice needs a concrete typed-array type"
    );
    let len = bytes.len();
    if len == 0 {
...
pub fn typed_array_from_vec(
    ...
    debug_assert!(
        ty != TypedArrayType::TypeNone && ty != TypedArrayType::TypeDataView,
        "typed_array_from_vec needs a concrete typed-array type"
    );
    if bytes.is_empty() {
