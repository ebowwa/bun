...
fn change_mode(patch_dir: Fd, change: &FileModeChange<'_>) -> sys::Result<()> {
    ...
    let _close = scopeguard::guard(fd, |fd| fd.close());
    let mode = change.new_mode.to_bun_mode();
    // On Windows `sys::fchmod` routes through libuv and panics on the
    // HANDLE-backed fd returned by `openat`; use the path-based `fchmodat`
    // against the verified parent there.
    if cfg!(windows) {
        sys::fchmodat(parent.fd, &base, mode, 0)
    } else {
        sys::fchmod(fd, mode)
    }
}
