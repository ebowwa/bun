...
use bun_jsc::{
    ...
    JsResult, Local, Node, Scope, StringJsc as _, VirtualMachine, host_fn,
};
...
#[bun_jsc::host_fn(export = "BunServe__onResolvePlugins", scoped)]
fn on_resolve_impl<'s>(scope: &mut Scope<'s>, callframe: &CallFrame) -> JsResult<Local<'s>> {
    ctx_log!("onResolve");
...
#[bun_jsc::host_fn(export = "BunServe__onRejectPlugins", scoped)]
fn on_reject_impl<'s>(scope: &mut Scope<'s>, callframe: &CallFrame) -> JsResult<Local<'s>> {
    ctx_log!("onReject");
    ...
    NewRequestContext<Self, SSL, DEBUG, true>: super::request_context::RequestContextHostFns,
{
    ...
    pub(crate) fn do_subscriber_count<'s>(
        this: &mut Self,
        scope: &mut Scope<'s>,
        callframe: &CallFrame,
    ) -> JsResult<Local<'s>> {
        let [topic_value] = callframe.scoped_arguments::<1>(scope).ptr;
        ...
        let topic = topic_value.to_slice(scope)?;

        if topic.slice().is_empty() {
            return Ok(scope.number(0.0));
        }

        Ok(scope.number(f64::from(this.app_mut().num_subscribers(topic.slice()))))
    }
    ...
    #[bun_jsc::host_fn(method, scoped)]
    pub(crate) fn dispose<'s>(
        this: &mut Self,
        scope: &mut Scope<'s>,
        _callframe: &CallFrame,
    ) -> JsResult<Local<'s>> {
        Ok(scope.local(this.dispose_from_js()))
    }

    /// `pub const doUpgrade = host_fn.wrapInstanceMethod(ThisServer, "onUpgrade", false)`
    #[bun_jsc::host_fn(method, scoped)]
    pub(crate) fn do_upgrade<'s>(
        this: &mut Self,
        scope: &mut Scope<'s>,
        callframe: &CallFrame,
    ) -> JsResult<Local<'s>> {
    ...
    pub(crate) fn do_reload(
        ...
        Self::on_reload(self, global, callframe)
    }
    ...
    pub(crate) fn do_fetch(
        ...
        Self::on_fetch(self, global, callframe)
    }
    ...
    pub(crate) fn do_timeout(
        ...
        Self::timeout(self, global, callframe)
    }
    ...
    pub(crate) fn timeout<'s>(
        ...
        if matches!(this.config.address, server_config::Address::Unix(_)) {
    ...
    pub(crate) fn on_reload<'s>(
        ...
        let mut args_slice = jsc::ArgumentsSlice::init(scope.unscoped_bun_vm(), arguments);
        ...
        let mut new_config = ServerConfig::from_js(
            ...
            server_config::FromJSOptions {
                ...
                previous_fetch: !this.config.on_request.is_empty_or_undefined_or_null(),
                previous_routes: !this.user_routes.is_empty(),
            },
        )?;
        ...
        this.on_reload_from_zig(&mut new_config, global);

        Ok(scope.local(this.js_value.try_get().unwrap_or(JSValue::UNDEFINED)))
    }

    #[bun_jsc::host_fn(method, scoped)]
    pub(crate) fn on_fetch<'s>(
        this: &mut Self,
        scope: &mut Scope<'s>,
        callframe: &CallFrame,
    ) -> JsResult<Local<'s>> {
        ...
        // SAFETY: bun_vm() returns the live per-thread VM singleton.
        let mut args = jsc::ArgumentsSlice::init(scope.unscoped_bun_vm(), arguments);
        ...
        let existing_request: Box<Request> = if first_arg.is_string() {
            ...
            let owned_url_buf: std::borrow::Cow<'_, [u8]> = if url.hostname.is_empty() {
                std::borrow::Cow::Owned(
                    strings::append(&this.base_url_string_for_joining, url.pathname).into_vec(),
                )
            } else {
            ...
            if arguments.len() >= 2 && arguments[1].is_object() {
                ...
                if let Some(body__) = opts.fast_get(ctx, jsc::BuiltinName::Body)? {
                    match Blob::get::<true, false>(ctx, body__) {
                        ...
                        Err(_) => {
                            return Ok(scope.local(JSPromise::dangerously_create_rejected_promise_value_without_notifying_vm(
                                ctx,
        ...
        debug_assert!(!this.config.on_request.is_empty()); // confirmed above
        let global_this = this.global();
        let on_request = this.config.on_request;
        ...
        let response_value =
            match on_request.call(&global_this, this.js_value_assert_alive(), &[request_value]) {
                Ok(v) => v,
        ...
        if response_value.as_any_promise().is_some() {
            return Ok(scope.local(response_value));
        }
        ...
        Ok(scope.local(JSPromise::resolved_promise_value(ctx, response_value)))
    }

    #[bun_jsc::host_fn(method, scoped)]
    pub(crate) fn close_idle_connections<'s>(
        this: &mut Self,
        scope: &mut Scope<'s>,
        _callframe: &CallFrame,
    ) -> JsResult<Local<'s>> {
        if this.app.is_none() || this.deinit_running.get() {
            return Ok(scope.number(0.0));
        }
        ...
        let closed = this.app_mut().close_idle_connections(false);
        this.deinit_running.set(false);
        this.deinit_if_we_can();
        Ok(scope.number(closed as f64))
    }
    ...
    pub(crate) fn get_port<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        let config_port = match &this.config.address {
            server_config::Address::Unix(_) => return Ok(scope.undefined()),
        ...
        if let Some(listener) = this.listener {
            ...
            if let Some(p) = bun_opaque::opaque_deref_mut(listener).get_local_port() {
                return Ok(scope.number(p as f64));
            }
        }
        if Self::HAS_H3 {
            if let Some(h3l) = this.h3_listener {
        ...
        Ok(scope.number(config_port as f64))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_id<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        scope.string_utf8(&this.config.id)
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_pending_requests<'s>(
        this: &Self,
        scope: &mut Scope<'s>,
    ) -> JsResult<Local<'s>> {
        Ok(scope.number((this.pending_requests.get() as u32 & 0x7FFF_FFFF) as i32 as f64))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_pending_web_sockets<'s>(
        this: &Self,
        scope: &mut Scope<'s>,
    ) -> JsResult<Local<'s>> {
        Ok(scope.number((this.active_sockets_count() as u32 & 0x7FFF_FFFF) as i32 as f64))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_address<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        let global = scope.unscoped_global();
        match &this.config.address {
            ...
            server_config::Address::Tcp { port: tcp_port, .. } => {
                ...
                if let Some(listener) = this.listener {
                    ...
                    return addr.into_dto(&this.global()).map(|v| scope.local(v));
                }
                if Self::HAS_H3 {
                    if let Some(h3l) = this.h3_listener {
                        // S008: `h3::ListenSocket` is an `opaque_ffi!` ZST — safe deref.
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_url<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        let global = scope.unscoped_global();
        let mut url = this
            .get_url_as_string()
                ...
                match &this.config.address {
                    server_config::Address::Tcp { hostname, .. } => {
                        if let Some(hostname) = hostname {
                            ...
                            return BunString::static_(b"localhost")
                                .to_js(global)
                                .map(|v| scope.local(v));
                        }
                    }
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_protocol<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        let _ = this;
        let global = scope.unscoped_global();
        if SSL {
            BunString::static_(b"https")
                .to_js(global)
                .map(|v| scope.local(v))
        } else {
            BunString::static_(b"http")
                .to_js(global)
                .map(|v| scope.local(v))
        }
    }
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_development<'s>(_this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        Ok(scope.local(JSValue::from(DEBUG)))
    }
