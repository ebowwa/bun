...
impl Expect {
        ...
        let existing_value = match runner.snapshots.get_or_put(this, &pretty_value, hint) {
            ...
            Err(err) => {
                ...
                return Err(match err {
                    crate::Error::FailedToOpenSnapshotFile => global_this.throw(format_args!(
                        "Failed to open snapshot file for test file: {test_file_path}"
                    )),
                    crate::Error::FailedToMakeSnapshotDirectory => global_this.throw(format_args!(
                        "Failed to make snapshot directory for test file: {test_file_path}"
                    )),
                    crate::Error::FailedToWriteSnapshotFile => global_this.throw(format_args!(
                        "Failed write to snapshot file: {test_file_path}"
                    )),
                    crate::Error::SyntaxError | crate::Error::ParseError => global_this.throw(
                        format_args!("Failed to parse snapshot file for: {test_file_path}"),
                    ),
                    ...
                    crate::Error::SnapshotInConcurrentGroup => global_this.throw(format_args!(
                        "Snapshot matchers are not supported in concurrent tests"
                    )),
                    crate::Error::TestNotActive => global_this.throw(format_args!(
    ...
    // extern shim emitted by `#[bun_jsc::JsClass]` codegen (TypeClass__construct/__call); bare `#[host_fn]` cannot target an associated fn without a receiver.
    pub(crate) fn object_containing(
        global_this: &JSGlobalObject,
        call_frame: &CallFrame,
    ) -> JsResult<JSValue> {
    ...
    // extern shim emitted by `#[bun_jsc::JsClass]` codegen (TypeClass__construct/__call); bare `#[host_fn]` cannot target an associated fn without a receiver.
    pub(crate) fn string_containing(
        global_this: &JSGlobalObject,
        call_frame: &CallFrame,
    ) -> JsResult<JSValue> {
    ...
    // extern shim emitted by `#[bun_jsc::JsClass]` codegen (TypeClass__construct/__call); bare `#[host_fn]` cannot target an associated fn without a receiver.
    pub(crate) fn apply_custom_matcher(
        global_this: &JSGlobalObject,
        call_frame: &CallFrame,
    ) -> JsResult<JSValue> {
    ...
    #[inline]
    pub(crate) fn add_snapshot_serializer(
        global_this: &JSGlobalObject,
        call_frame: &CallFrame,
    ) -> JsResult<JSValue> {
    ...
    // extern shim emitted by `#[bun_jsc::JsClass]` codegen (TypeClass__construct/__call); bare `#[host_fn]` cannot target an associated fn without a receiver.
    pub(crate) fn not_implemented_static_fn(
        global_this: &JSGlobalObject,
        _: &CallFrame,
    ) -> JsResult<JSValue> {
                ...
                if let Some(rparen) = strings::index_of_char_pos(source, b')', lparen) {
                    ...
                    for param_name in strings::split(params_str, b",") {
                        if param_index > 0 {
                            ...
                            if param_index > 1 {
                                if self.colors {
                                    writer.write_str(bun_core::pretty_fmt!(
                                        "<r><d>, <r><green>",
                                        true
                                    ))?;
                                } else {
...
impl ExpectMatcherContext {
    ...
    pub(crate) fn get_utils<'s>(_this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        // SAFETY: FFI call with valid &JSGlobalObject
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_is_not<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        Ok(scope.boolean(this.flags.not()))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_promise<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_expand<'s>(_this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
...
impl ExpectTypeOf {
    ...
    #[bun_jsc::host_fn(method)]
    pub(crate) fn fn_one_argument_returns_void(
        &self,
        _: &JSGlobalObject,
        _: &CallFrame,
    ) -> JsResult<JSValue> {
    ...
    #[bun_jsc::host_fn(method)]
    pub(crate) fn fn_one_argument_returns_expect_type_of(
        &self,
        global_this: &JSGlobalObject,
        _: &CallFrame,
    ) -> JsResult<JSValue> {
    ...
    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_returns_expect_type_of<'s>(
        _this: &Self,
        scope: &mut Scope<'s>,
    ) -> JsResult<Local<'s>> {
