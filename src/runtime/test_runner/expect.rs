...
pub mod mock {
    ...
    // `<tag>` markup lives in the template literals here; the `lead` /
    // `prefix` / `which` / `tail` pieces are substituted as positional args,
    // so (like user data) they are emitted verbatim and must be plain text.
