...
//   undefined/null arguments, `strict` throws on them (and optionally takes
//   the `CommandMeta` to send with), and `required "arg"` additionally throws
//   when the first argument is missing (implies skip_null).
...
macro_rules! cmd_varargs {
    ($fn_name:ident, $name:literal, $command:literal, required $arg0_name:literal, $state:ident $(,)?) => {
        cmd_varargs!(@impl $fn_name, $name, $command, true, $state, CommandMeta::default(), $arg0_name);
    };
    ($fn_name:ident, $name:literal, $command:literal, skip_null, $state:ident $(,)?) => {
        cmd_varargs!(@impl $fn_name, $name, $command, true, $state, CommandMeta::default());
    };
    ($fn_name:ident, $name:literal, $command:literal, strict, $state:ident $(,)?) => {
        cmd_varargs!(@impl $fn_name, $name, $command, false, $state, CommandMeta::default());
    };
    ($fn_name:ident, $name:literal, $command:literal, strict, $state:ident, $meta:expr $(,)?) => {
        cmd_varargs!(@impl $fn_name, $name, $command, false, $state, $meta);
    };
    (@impl $fn_name:ident, $name:literal, $command:literal, $skip_null:literal, $state:ident, $meta:expr $(, $arg0_name:literal)?) => {
        #[bun_jsc::host_fn(method)]
        pub fn $fn_name(
            this: &Self,
            global: &JSGlobalObject,
