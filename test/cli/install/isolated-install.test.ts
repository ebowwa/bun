...
describe("bun link integration", () => {
  ...
  test("isolated: scoped dep honors active bun link", async () => {
    // Scoped registrations live one level deeper in the global link dir
    // (`<linkdir>/@scope/name`); the link scan flattens them to
    // `@scope/name` cache keys via a separate branch from the unscoped
    // path, so cover it with a scoped producer.
    using home = tempDir("link-home-", {});
    const env = hermeticEnv(String(home));
    using producer = tempDir("linkpkg-scoped-producer-", {
      "package.json": JSON.stringify({ name: "@types/is-number", version: "1.0.0" }),
      "index.js": "module.exports = 'FROM_PRODUCER';",
      "marker.js": "module.exports = 'FROM_PRODUCER_MARKER';",
    });
    await using linkProc = spawn({
    ...
    await write(
      packageJson,
      JSON.stringify({
        name: "isolated-link-consumer-scoped",
    ...
    // Assert through the top-level symlink so the store-path encoding for
    // scoped names stays an implementation detail.
    const topLevel = join(packageDir, "node_modules", "@types", "is-number");
    expect(await file(join(topLevel, "marker.js")).text()).toContain("FROM_PRODUCER_MARKER");
    expect(await file(join(topLevel, "index.js")).text()).toContain("FROM_PRODUCER");
  });
  ...
  test("isolated: bun unlink restores registry contents on reinstall", async () => {
    // Default config keeps the entry project-local (no global virtual
    // store), so without the `.bun-link` marker the reinstall's existence
    // check would pass through the producer-copied body and keep the stale
    // `marker.js` forever.
    using home = tempDir("link-home-", {});
    const env = hermeticEnv(String(home));
    using producer = await setupLinkedNoDeps(env);
    ...
    const bodyDir = join(packageDir, "node_modules", ".bun", "no-deps@1.0.0", "node_modules", "no-deps");
    expect(existsSync(join(bodyDir, "marker.js"))).toBe(true);
    expect(existsSync(join(bodyDir, ".bun-link"))).toBe(true);

    await using unlinkProc = spawn({
      cmd: [bunExe(), "unlink"],
      cwd: String(producer),
      env,
      stdout: "pipe",
    ...
    const [, unlinkStderr, unlinkExit] = await Promise.all([
      unlinkProc.stdout.text(),
      unlinkProc.stderr.text(),
      unlinkProc.exited,
    ]);
    expect(unlinkStderr).not.toContain("error:");
    expect(unlinkExit).toBe(0);

    await runInstall();

    expect(existsSync(join(bodyDir, "marker.js"))).toBe(false);
    expect(existsSync(join(bodyDir, ".bun-link"))).toBe(false);
    expect(existsSync(join(bodyDir, "package.json"))).toBe(true);
  });

  test("isolated: global install with isolated linker is not treated as bun link", async () => {
    using home = tempDir("link-home-", {});
    const env = hermeticEnv(String(home));

    // `bun add -g` with the isolated linker drops
    // `<global>/node_modules/no-deps` as a symlink into the global dir's
    // own `.bun` store. Link detection must not treat that as a
    // `bun link` registration, or every consumer project would get the
    // globally installed version copied over its lockfile-resolved one.
    using globalCwd = tempDir("global-add-cwd-", {});
    await write(
      join(String(home), ".bunfig.toml"),
      Bun.TOML.stringify({
        install: { linker: "isolated", registry: registry.registryUrl() },
      }),
    );
    await using addProc = spawn({
      cmd: [bunExe(), "add", "-g", "no-deps@2.0.0"],
      cwd: String(globalCwd),
      env,
      stdout: "pipe",
      stderr: "pipe",
    });
    const [addStdout, addStderr, addExit] = await Promise.all([
      addProc.stdout.text(),
      addProc.stderr.text(),
      addProc.exited,
    ]);
    expect(addStderr).not.toContain("error:");
    expect(addExit).toBe(0);
    // Sanity: the global entry landed as a symlink, the exact shape the
    // link scan must reject. (Windows materializes a junction; the
    // per-call attribute check there is covered by the same
    // target-resolution rejection.)
    const globalEntry = join(String(home), "install", "global", "node_modules", "no-deps");
    if (!isWindows) {
      expect(lstatSync(globalEntry).isSymbolicLink()).toBe(true);
    }
    void addStdout;
    ...
    const bodyDir = join(packageDir, "node_modules", ".bun", "no-deps@1.0.0", "node_modules", "no-deps");
    expect(await file(join(bodyDir, "package.json")).json()).toMatchObject({ version: "1.0.0" });
    expect(existsSync(join(bodyDir, ".bun-link"))).toBe(false);
  });

  test("isolated: globalStore global install is not treated as bun link", async () => {
    // With `globalStore` enabled the global install's store entry is an
    // absolute symlink into `<cache>/links/`, so the top-level entry's
    // full resolution escapes the link dir. The scan must discriminate on
    // the immediate symlink target (relative `.bun/...`), not the
    // resolved path, or the global version substitutes into consumers.
    using home = tempDir("link-home-", {});
    const env = hermeticEnv(String(home));
    ...
    await write(
      join(String(home), ".bunfig.toml"),
      Bun.TOML.stringify({
        install: { linker: "isolated", globalStore: true, registry: registry.registryUrl() },
      }),
    );
    await using addProc = spawn({
      cmd: [bunExe(), "add", "-g", "no-deps@2.0.0"],
    ...
    const [, addStderr, addExit] = await Promise.all([addProc.stdout.text(), addProc.stderr.text(), addProc.exited]);
    expect(addStderr).not.toContain("error:");
    expect(addExit).toBe(0);
    ...
    await write(
      packageJson,
      JSON.stringify({
        name: "isolated-link-consumer-gvs",
        dependencies: { "no-deps": "1.0.0" },
      }),
    );
  ...
  test("isolated: bun add -g installs registry bytes for a link-registered name", async () => {
    // A global install IS the link dir: `bun add -g` of a name that is
    // also link-registered must install registry bytes, not the
    // producer's working tree (hoisted parity: the registration gets
    // clobbered, not sourced).
    using home = tempDir("link-home-", {});
    const env = hermeticEnv(String(home));
    using producer = await setupLinkedNoDeps(env);
    ...
    await using addProc = spawn({
      cmd: [bunExe(), "add", "-g", "no-deps@1.0.0"],
      cwd: String(globalCwd),
      env,
      stdout: "pipe",
      stderr: "pipe",
    ...
    expect(existsSync(join(globalBody, "package.json"))).toBe(true);
    expect(existsSync(join(globalBody, "marker.js"))).toBe(false);
    expect(existsSync(join(globalBody, ".bun-link"))).toBe(false);
    void producer;
  });
  ...
  test("isolated: no link registered → body sourced from registry tarball", async () => {
    ...
    expect(existsSync(join(bodyDir, "package.json"))).toBe(true);
    expect(existsSync(join(bodyDir, "marker.js"))).toBe(false);
    // The linked-names probe is read-only: an install on a machine that
    // never ran `bun link` must not create the global link dir tree.
    expect(existsSync(join(String(home), "install", "global"))).toBe(false);
  });
  ...
  //
  // Windows dropped: the staged dangling state and the lstat
  // isSymbolicLink() assertions assume the registration is a symlink.
  // On Windows `bun link` registers a junction and the installer detects
  // dangling registrations through the GetFileAttributesW probe in
  // linked_package_path, a different path this POSIX-shaped fixture
  // doesn't validate.
  ...
  test("isolated: link overrides only the direct-dep resolution, not other versions of the name", async () => {
    using home = tempDir("link-home-", {});
    const env = hermeticEnv(String(home));
    using producer = await setupLinkedNoDeps(env);
