...
test.concurrent("node:module: linking constructs the linked bindings, not the rest of the table", async () => {
  const result = await runEntry(`
    ...
    const exportNames = Reflect.ownKeys(ns).filter(key => typeof key === "string");
    // The export list is the static table's enumerable entries, which is also exactly what Object.keys of the
    // constructor lists. The non-enumerable entries are not exports, as in Node.
